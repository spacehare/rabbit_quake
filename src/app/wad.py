...
class WAD:
    ...
    def from_folder(folder: Path):
        ...
        for path in image_paths:
            ...
            with Image.open(path) as img:
                ...
                tex: MipTex = MipTex(
                    name.encode(), img.width, img.height, mip_offs, mipmaps
                )
                new_wad.dir_offset += tex.size_in_wad
                entry: Entry = Entry(
                    current_entry_offset,
                    current_mipmap_offset,
                    current_mipmap_offset,
                    "D",
                    0,
                    name.encode(),
    ...
    def from_wad_file(path: Path | str):
        ...
        with open(path, "rb") as f:
            id, numentries, dir_offs = WAD.STRUCT_HEADER.unpack(
                f.read(WAD.STRUCT_HEADER.size)
            )
            new_wad.wad_id = id
            ...
            for _ in range(numentries):
                ...
                mip_offsets = struct.unpack(
                    "4I", f.read(16)
                )  # offset from start of texture file
...
class Entry:
    ...
    def from_lump(lump: bytes):
        ...
        offset, disk_size, size, entry_type, compression, _, raw_name = (
            Entry.STRUCT.unpack(lump)
        )
...
class MipTex:
    ...
    def __repr__(self) -> str:
        return (
            f"MipTex(name='{self.name}', width={self.width}, height={self.height}, "
    ...
    def size_in_wad(self):
        return MipTex.STRUCT_NO_MIPMAPS.size + sum(
            [m.height * m.width for m in self.mipmaps]
        )
    ...
    @property
    def to_bytes(self) -> bytes:
        return self.STRUCT_NO_MIPMAPS.pack(
            self.name, self.width, self.height, *self.mip_offsets
        )
...
if __name__ == "__main__":
    ...
    for n, w in [("folder", wadfolder), ("wad", wadfile)]:
        ...
        print("-----", n)
        print(
            f"dir_offs: {w.dir_offset} - entries: {len(w.entries)} - textures: {len(w.textures)}"
        )
        print("  entries:")
        for e in w.entries:
            print(
                f"    - {e.name} -", "offs:", e.offset, "- sizes:", e.disk_size, e.size
            )
        print("  textures:")
        for t in w.textures:
            print(
