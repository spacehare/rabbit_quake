...
def if_tb_open(func):
    def wrapper(*args):
        if a.win_is_active(TB_AHK_TITLE) and callable(func):
            ...
            print(
                f"attempted to run {func.__name__}, but trenchbroom was not the active window"
            )
...
def find_map_from_tb_title():
    ...
    if tb:
        ...
        if tb_qmap_name_search:
            ...
            print(
                "name from TrenchBroom window title:",
                colorize(tb_qmap_name, bcolors.OKCYAN),
            )
...
def compile_and_transfer(profile, compiler, qbsp, vis, light):
    if current_map:
        ...
        ret = ericw.compile(
            profile, compiler, current_map, qbsp=qbsp, vis=vis, light=light
        )
        transfer()
        return ret
...
if __name__ == "__main__":
    ...
    a.add_hotkey(
        Keybinds.compile,
        lambda: compile_and_transfer(
            current_profile, current_compiler, enable_qbsp, enable_vis, enable_light
        ),
    )
    a.add_hotkey(
        "!b", lambda: compile_then_launch(enable_qbsp, enable_vis, enable_light)
    )
